...
export type WorkCategoryId = 'platform' | 'prototype' | 'planned' | 'opensource';
...
export const workCategories: {
  ...
  },
  {
    id: 'opensource',
    title: '开源项目',
    intro: '独立开发并发布的开源项目，代码托管于 GitHub、可通过 npm 安装使用，面向开发者与 AI 用户社区。',
  },
];
...
export const workProjects: WorkProject[] = [
  {
    ...
    caseStudy: {
      ...
      results:
        '交付 <strong>投放管理+数据管理+风控一体化投放平台</strong>，年均管理超 <strong>3000 万美元</strong>广告预算，覆盖 <strong>1000+</strong> 广告账户，支撑 10+ 人投放团队日常运作。形成从投放管理、数据策略到风控结算的完整闭环，<strong>AI 建议与一键应用</strong>缩短「看数—判断—改价」路径。基于平台数据与策略沉淀，完成<strong>智能投放 Agent 方案设计</strong>，为 AI 驱动投放演进提供储备。与波波等前序数据/投放类产品形成同一产品线上的能力递进。',
    },
  },
  {
  ...
  },
  {
    ...
    tooltipSummary:
      '基于MCP协议的上下文感知AI Agent产品，产品已上线供内部投放团队使用。',
    tagline:
    ...
    caseStudy: {
      ...
      process:
        '作为<strong>产品负责人</strong>，主导产品架构设计与方案落地。完成MCP协议与工具调用框架的技术选型与方案设计，定义上下文注入机制与动态工具映射规则。设计混合交互流程——Agent建议以tool_call返回，前端捕获后渲染为确认弹窗/参数编辑表单等交互组件。完成系统提示词编写，定义Agent行为边界与异常兜底策略。推动开发团队完成MCP Server开发，统筹前后端协作，产品已上线并在内部投放团队推广培训。',
      results:
        '产品已<strong>上线</strong>，面向内部投放团队使用，正在推广培训。上线前内测数据表现：<strong>关键词选词效率提升 75%+、单账户调优耗时缩短 67%+、运营操作量降低 87%+</strong>。基于 MCP 协议的工具调用框架、上下文感知注入机制与混合交互模式，在真实投放场景中跑通「AI 建议→人工确认→自动执行」的人机协作闭环。',
    },
  },
  {
  ...
  },
  {
    ...
    demoPath: null,
    demoExternalUrl: 'https://github.com/leonardwwq/dsh-cultivation',
    iframeTitle: 'dsh-cultivation GitHub 仓库',
    prototypesFolder: 'dsh-cultivation',
    caseStudy: {
      background:
        'DSH（DeepSeek Harness）是面向 Agent 的 Web 界面。日常使用中，每次对话都是「全新」的——没有持续的人设、没有关系积累，记忆只能靠把历史上下文硬塞给模型，<strong>成本高且容易丢失</strong>。希望 AI 陪伴有「养成感」：角色有完整背景、会随对话成长、记得彼此之间发生过什么，且<strong>数据完全本地、不上传、不浪费 token</strong>。',
      goals:
        '做一个可直接安装到 DSH Web 的插件，覆盖五块能力：<strong>① 会话级角色绑定</strong>——新会话选择角色后锁定（仅可开关）；<strong>② 角色档案</strong>——完整人设（成长经历/教育/职业/创伤/秘密等），内置 AI 设计助手（设计师/访谈）辅助生成；<strong>③ 好感度系统</strong>——随对话自然变化、分五阶段（冷淡 → 客气 → 熟络 → 亲密 → 依赖）、语气与称呼随阶段调整、由系统维护不可直接修改；<strong>④ 记忆系统</strong>——分区存储（长期事实/用户信息/关系状态/承诺/事件）+ 追加式日志自动归档，跨会话累积且防丢失；<strong>⑤ 资源纪律</strong>——角色段仅注入根会话、子代理不受影响，不浪费 token。',
      process:
